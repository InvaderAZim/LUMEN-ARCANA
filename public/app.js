...
function setNav(){const items=[['home','◉','Сьогодні'],['reading','✦','Розклад'],['library','▤','Бібліотека'],['deck','▦','Колода'],['journal','✎','Щоденник'],['profile','☾','Профіль']];nav.hidden=false;nav.innerHTML=items.map(([r,i,l])=>`<button class="nav-btn ${state.route===r?'active':''}" data-r="${r}"${state.route===r?' aria-current="page"':''}><b>${i}</b><span>${l}</span></button>`).join('');nav.querySelectorAll('[data-r]').forEach(b=>b.onclick=()=>navigate(b.dataset.r))}
