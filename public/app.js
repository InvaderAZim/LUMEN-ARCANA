...
function natal(){const c=shell('Натальна карта','Beta · дані народження та базовий сонячний профіль'),saved=safeParseLocal('la_natal_profile',{}),sign=sunSign(saved.date);c.innerHTML=`<section class="premium-panel"><div class="badge">NATAL · BETA</div><h2>Твої дані народження</h2><p class="lumen-muted-copy">Зараз модуль розраховує базовий сонячний знак. Планети, будинки й аспекти не вигадуються без повного астрономічного розрахунку.</p><div class="settings-grid natal-input-grid"><article><small>ДАТА</small><label>Дата народження</label><input id="birthDate" type="date" value="${esc(saved.date||'')}" class="lumen-field"></article><article><small>ЧАС</small><label>Час народження</label><input id="birthTime" type="time" value="${esc(saved.time||'')}" class="lumen-field"></article><article><small>МІСЦЕ</small><label>Місто</label><input id="birthPlace" value="${esc(saved.place||'')}" class="lumen-field"></article><article><small>СТАТУС</small><h3>${sign?`${sign[1]} ${sign[0]}`:'Дані не збережені'}</h3><p>Дані зберігаються на цьому пристрої.</p></article></div><button class="primary wide" id="saveNatal">Зберегти й розрахувати</button></section>`;document.querySelector('#saveNatal').onclick=()=>{const p={date:birthDate.value,time:birthTime.value,place:birthPlace.value.trim()};if(!p.date)return flash('Вкажи дату народження');localStorage.setItem('la_natal_profile',JSON.stringify(p));natal()}}
...
function library(){const c=shell('Бібліотека знань','Практичний довідник LUMEN ARCANA');if(state.libraryTopic){const t=LIBRARY.find(x=>x[0]===state.libraryTopic)||LIBRARY[0];c.innerHTML=`<section class="premium-panel"><div class="badge">LIBRARY · PREMIUM</div><h2>${t[1]} ${t[2]}</h2><p class="lumen-muted-copy">${t[3]}</p><div class="reading-cards">${t[4].map((x,i)=>`<article><div class="card-no">0${i+1}</div><p>${esc(x)}</p></article>`).join('')}</div><div class="result-actions"><button class="ghost" id="backLibrary">← До бібліотеки</button>${t[0]==='major'?'<button class="primary" id="openDeck">Відкрити колоду</button>':''}</div></section>`;document.querySelector('#backLibrary').onclick=()=>{state.libraryTopic=null;library()};const d=document.querySelector('#openDeck');if(d)d.onclick=()=>{state.route='deck';state.libraryTopic=null;render()};return}c.innerHTML=`<section class="premium-strip"><span>LIBRARY</span><b>${LIBRARY.length} тематичних розділів</b><small>Premium beta</small></section><section class="settings-grid library-grid">${LIBRARY.map(t=>`<article><small class="library-section-label">${LIBRARY_SECTION_LABELS[t[0]]||'РОЗДІЛ'}</small><h3>${t[2]}</h3><p>${t[3]}</p><button class="text-btn" data-topic="${t[0]}">Читати →</button></article>`).join('')}</section><section class="day-card"><div><small>НАВЧАННЯ</small><h3>Академія · 6 уроків</h3><p>Послідовна практика: питання, символи, позиції, зв’язки, щоденник і межі тлумачення.</p></div><button class="ghost" id="openAcademy">Відкрити</button></section>`;c.querySelectorAll('[data-topic]').forEach(b=>b.onclick=()=>{state.libraryTopic=b.dataset.topic;library()});document.querySelector('#openAcademy').onclick=()=>{state.route='academy';render()}}
function academy(){const c=shell('Академія','Усі уроки відкриті у beta.');c.innerHTML=`<section class="premium-strip"><span>PREMIUM</span><b>6 / 6 уроків доступно</b><small>Без лімітів</small></section><section class="lesson-list">${LESSONS.map(([n,t,d,p])=>{const practiceId=`academy-practice-${n}`;return`<article class="lesson"><div class="lesson-no">${n}</div><div><h3>${t}</h3><p>${d}</p><button class="text-btn" data-academy-practice aria-expanded="false" aria-controls="${practiceId}">Відкрити практику →</button><div class="extra" id="${practiceId}" aria-hidden="true"><p class="academy-practice"><strong>Практика:</strong> ${esc(p)}</p></div></div></article>`}).join('')}</section>`}
