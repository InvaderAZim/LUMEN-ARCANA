...
function moon(){const m=moonFallbackData(),fmt=d=>d.toLocaleDateString('uk-UA',{day:'2-digit',month:'long'}),c=shell('Місячний календар','Базовий цикл · fallback режим');c.innerHTML=`<section class="hero-premium"><div class="hero-copy"><div class="badge" title="Резервний локальний розрахунок місячної фази">LUNAR · FALLBACK</div><h2>${m.symbol} ${m.name}</h2><p>Вік циклу: <b>${m.age.toFixed(1)} дня</b> · освітленість: <b>${Math.round(m.illum)}%</b>.</p><div class="synthesis"><small>ФОКУС ФАЗИ</small><p>${m.focus}</p></div></div><div class="arcana-mark"><span>MOON</span><strong>${m.symbol}</strong><small>${Math.round(m.p*100)}% ЦИКЛУ</small></div></section><section class="settings-grid lumen-mt-14"><article><small>🌑 НОВИЙ МІСЯЦЬ</small><h3>${fmt(m.newM)}</h3><p>Початок нового синодичного циклу.</p></article><article><small>🌓 ПЕРША ЧВЕРТЬ</small><h3>${fmt(m.first)}</h3><p>Приблизно 25% місячного циклу.</p></article><article><small>🌕 ПОВНЯ</small><h3>${fmt(m.full)}</h3><p>Приблизно 50% місячного циклу.</p></article><article><small>🌗 ОСТАННЯ ЧВЕРТЬ</small><h3>${fmt(m.last)}</h3><p>Приблизно 75% місячного циклу.</p></article></section><section class="day-card"><div><small>РЕЖИМ FALLBACK</small><h3>Базове астрономічне наближення</h3><p>Місячний календар продовжує працювати навіть без додаткового модуля. Для enhanced-версії використовується extensions.js.</p></div><div class="mini-card" aria-hidden="true"><span>MOON</span><b>${m.symbol}</b></div></section>`}
