...
function academy(){const c=shell('Академія','Усі уроки відкриті у beta.');c.innerHTML=`<section class="premium-strip"><span>PREMIUM</span><b>6 / 6 уроків доступно</b><small>Без лімітів</small></section><section class="lesson-list">${LESSONS.map(([n,t,d,p])=>`<article class="lesson"><div class="lesson-no">${n}</div><div><h3>${t}</h3><p>${d}</p><button class="text-btn" onclick="this.parentElement.querySelector('div.extra').classList.toggle('open')">Відкрити практику →</button><div class="extra"><p class="academy-practice"><strong>Практика:</strong> ${esc(p)}</p></div></div></article>`).join('')}</section>`}
