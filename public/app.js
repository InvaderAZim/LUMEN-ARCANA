...
function compatibility(){const c=shell('Сумісність','Бета · порівняння двох сонячних знаків'),me=safeParseLocal('la_natal_profile',{}),saved=safeParseLocal('la_compatibility',{}),myDate=me.date||saved.myDate||'',partnerDate=saved.partnerDate||'',a=sunSign(myDate),b=sunSign(partnerDate),result=a&&b?compatibilityText(a[0],b[0]):null;c.innerHTML=`<section class="premium-panel"><div class="badge">СУМІСНІСТЬ · БЕТА · ПРЕМІУМ</div><h2>Порівняй дві дати народження</h2><div class="settings-grid"><article><small>ТИ</small><label for="myDate">Моя дата народження</label><input id="myDate" type="date" aria-label="Моя дата народження" value="${esc(myDate)}" class="lumen-field"></article><article><small>ІНША ЛЮДИНА</small><label for="partnerDate">Дата народження партнера</label><input id="partnerDate" type="date" aria-label="Дата народження партнера" value="${esc(partnerDate)}" class="lumen-field"></article></div><button type="button" class="primary wide" id="calcCompatibility">Порівняти</button></section>${result?`<section class="day-card"><div><small>СУМІСНІСТЬ · ${result.score}%</small><h3>${a[1]} ${a[0]} + ${b[1]} ${b[0]}</h3><p>${result.text} Це рефлексивна оцінка, а не прогноз стосунків.</p></div><div class="mini-card" aria-hidden="true"><span>ПАРА</span><b>♡</b></div></section>`:''}`;document.querySelector('#calcCompatibility').onclick=()=>{const d={myDate:document.querySelector('#myDate').value,partnerDate:document.querySelector('#partnerDate').value};if(!d.myDate||!d.partnerDate)return flash('Вкажи обидві дати');localStorage.setItem('la_compatibility',JSON.stringify(d));compatibility()}}
...
function moon(){const m=moonFallbackData(),fmt=d=>d.toLocaleDateString('uk-UA',{day:'2-digit',month:'long'}),c=shell('Місячний календар','Базовий цикл · резервний режим');c.innerHTML=`<section class="hero-premium"><div class="hero-copy"><div class="badge" title="Резервний локальний розрахунок місячної фази">МІСЯЦЬ · РЕЗЕРВНИЙ РЕЖИМ</div><h2>${m.symbol} ${m.name}</h2><p>Вік циклу: <b>${m.age.toFixed(1)} дня</b> · освітленість: <b>${Math.round(m.illum)}%</b>.</p><div class="synthesis"><small>ФОКУС ФАЗИ</small><p>${m.focus}</p></div></div><div class="arcana-mark"><span>МІСЯЦЬ</span><strong>${m.symbol}</strong><small>${Math.round(m.p*100)}% ЦИКЛУ</small></div></section><section class="settings-grid lumen-mt-14"><article><small>🌑 НОВИЙ МІСЯЦЬ</small><h3>${fmt(m.newM)}</h3><p>Початок нового синодичного циклу.</p></article><article><small>🌓 ПЕРША ЧВЕРТЬ</small><h3>${fmt(m.first)}</h3><p>Приблизно 25% місячного циклу.</p></article><article><small>🌕 ПОВНЯ</small><h3>${fmt(m.full)}</h3><p>Приблизно 50% місячного циклу.</p></article><article><small>🌗 ОСТАННЯ ЧВЕРТЬ</small><h3>${fmt(m.last)}</h3><p>Приблизно 75% місячного циклу.</p></article></section><section class="day-card"><div><small>РЕЗЕРВНИЙ РЕЖИМ</small><h3>Базове астрономічне наближення</h3><p>Місячний календар продовжує працювати навіть без додаткового модуля. Для enhanced-версії використовується extensions.js.</p></div><div class="mini-card" aria-hidden="true"><span>МІСЯЦЬ</span><b>${m.symbol}</b></div></section>`}

function library(){const c=shell('Бібліотека знань','Практичний довідник LUMEN ARCANA');if(state.libraryTopic){const t=LIBRARY.find(x=>x[0]===state.libraryTopic)||LIBRARY[0];c.innerHTML=`<section class="premium-panel"><div class="badge">БІБЛІОТЕКА · БЕТА · ПРЕМІУМ</div><h2>${t[1]} ${t[2]}</h2><p class="lumen-muted-copy">${t[3]}</p><div class="reading-cards">${t[4].map((x,i)=>`<article><div class="card-no">0${i+1}</div><p>${esc(x)}</p></article>`).join('')}</div><div class="result-actions"><button type="button" class="ghost" id="backLibrary">← До бібліотеки</button>${t[0]==='major'?'<button type="button" class="primary" id="openDeck">Відкрити колоду</button>':''}</div></section>`;document.querySelector('#backLibrary').onclick=()=>{state.libraryTopic=null;library()};const d=document.querySelector('#openDeck');if(d)d.onclick=()=>{state.route='deck';state.libraryTopic=null;render()};return}c.innerHTML=`<section class="premium-strip"><span>БІБЛІОТЕКА</span><b>${LIBRARY.length} тематичних розділів</b><small>БЕТА · ПРЕМІУМ</small></section><section class="settings-grid library-grid">${LIBRARY.map(t=>`<article><small class="library-section-label">${LIBRARY_SECTION_LABELS[t[0]]||'РОЗДІЛ'}</small><h3>${t[2]}</h3><p>${t[3]}</p><button type="button" class="text-btn" data-topic="${t[0]}">Читати →</button></article>`).join('')}</section><section class="day-card"><div><small>НАВЧАННЯ</small><h3>Академія · 6 уроків</h3><p>Послідовна практика: питання, символи, позиції, зв’язки, щоденник і межі тлумачення.</p></div><button type="button" class="ghost" id="openAcademy">Відкрити</button></section>`;c.querySelectorAll('[data-topic]').forEach(b=>b.onclick=()=>{state.libraryTopic=b.dataset.topic;library()});document.querySelector('#openAcademy').onclick=()=>{state.route='academy';render()}}
function academy(){const c=shell('Академія','Усі уроки відкриті у бета-версії.');c.innerHTML=`<section class="premium-strip"><span>ПРЕМІУМ</span><b>6 / 6 уроків доступно</b><small>Без лімітів</small></section><section class="lesson-list">${LESSONS.map(([n,t,d,p])=>{const practiceId=`academy-practice-${n}`;return`<article class="lesson"><div class="lesson-no">${n}</div><div><h3>${t}</h3><p>${d}</p><button type="button" class="text-btn" data-academy-practice aria-expanded="false" aria-controls="${practiceId}">Відкрити практику →</button><div class="extra" id="${practiceId}" aria-hidden="true"><p class="academy-practice"><strong>Практика:</strong> ${esc(p)}</p></div></div></article>`}).join('')}</section>`}
...
function journal(){const c=shell('Щоденник','Особистий простір збережених розкладів.'),j=safeParseLocal('la_journal',[]);c.innerHTML=`<section class="premium-strip"><span>ПРИВАТНО</span><b>Дані зберігаються на цьому пристрої</b></section>${j.length?`<section class="journal-list">${j.map(x=>`<article class="journal-item"><time>${esc(x.createdAt||'')}</time><h3>${esc(x.title||x.question||'Розклад')}</h3><p>${esc(x.synthesis||'')}</p></article>`).join('')}</section>`:`<section class="empty-state"><div>✎</div><h2>Щоденник поки порожній</h2><p>Збережи перший розклад — він з’явиться тут.</p><button type="button" class="primary" id="toReading">Зробити розклад</button></section>`}`;const b=document.querySelector('#toReading');if(b)b.onclick=()=>{state.route='reading';render()}}
function profile(){const c=shell('Профіль','Преміум-доступ активовано автоматично на час бета-версії.'),name=tg?.initDataUnsafe?.user?.first_name||'LUMEN user';c.innerHTML=`<section class="profile-card"><div class="avatar">☾</div><div><h2>${esc(name)}</h2><p>Telegram Mini App</p></div><span class="premium-chip">БЕТА · ПРЕМІУМ</span></section><section class="settings-grid profile-stats-grid"><article><small>ДОСТУП</small><h3>Преміум</h3><p>Усі функції відкриті без оплати до завершення бета-версії.</p></article><article><small>РЕЖИМ</small><h3>${state.mode==='pro'?'Профі':'Початківець'}</h3><p>Змінюється у розділі «Розклад».</p></article><article><small>ШІ</small><h3>Гібридний</h3><p>ШІ працює за наявності API; інакше використовується локальний резервний режим.</p></article><article><small>БЕЗПЕКА</small><h3>Лише рефлексія</h3><p>Без фатальних прогнозів і підміни професійних рішень.</p></article></section>`}
