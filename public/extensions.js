...
function classicExportSvgX(data){
  ...
  const cell=52,labelW=138,mx=150,my=matrixY+100;
  ...
  const tableBottom=tableY+92+(keys.length+(asc!=null?1:0)+(mc!=null?1:0))*rowH;
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}"><rect width="${W}" height="${H}" fill="#fff"/><text x="90" y="66" font-size="18" letter-spacing="4" font-weight="700" fill="#8b6b2d">LUMEN ARCANA · CLASSIC NATAL CHART</text><text x="90" y="112" font-size="34" font-family="Georgia,serif" fill="#111">Натальна карта</text><text x="90" y="145" font-size="16" fill="#444">${info||'Персональні дані не вказані'}</text><text x="90" y="174" font-size="14" fill="#666">Система домів: ${escX(system)} · планетний engine: low-precision local</text><line x1="90" y1="195" x2="1310" y2="195" stroke="#bfa363"/><svg x="340" y="205" width="720" height="720" viewBox="0 0 720 720">${wheel.innerHTML}</svg><text x="90" y="${tableY}" font-size="25" font-family="Georgia,serif" fill="#111">Положення планет</text><text x="90" y="${tableY+42}" font-size="14" font-weight="700" fill="#666">ОБ’ЄКТ</text><text x="520" y="${tableY+42}" font-size="14" font-weight="700" fill="#666">ЗНАК</text><text x="850" y="${tableY+42}" font-size="14" font-weight="700" fill="#666">ПОЗИЦІЯ</text><text x="1190" y="${tableY+42}" font-size="14" font-weight="700" fill="#666">ДІМ</text><line x1="90" y1="${tableY+55}" x2="1310" y2="${tableY+55}" stroke="#888"/>${rows}${angleRows}<text x="90" y="${matrixY}" font-size="25" font-family="Georgia,serif" fill="#111">Матриця аспектів</text><text x="90" y="${matrixY+35}" font-size="13" fill="#1747ff">✶ △ гармонійні</text><text x="290" y="${matrixY+35}" font-size="13" fill="#ef1717">□ ☍ напружені</text><text x="490" y="${matrixY+35}" font-size="13" fill="#222">☌ з’єднання</text>${mHead}${mRows}<text x="90" y="${H-36}" font-size="12" fill="#777">LUMEN ARCANA · ${escX(system)} · Export</text></svg>`
}
function exportPngX(svgMarkup){
  ...
  img.onload=()=>{const c=document.createElement('canvas');c.width=1400;c.height=2110;const x=c.getContext('2d');x.fillStyle='#fff';x.fillRect(0,0,c.width,c.height);x.drawImage(img,0,0,c.width,c.height);URL.revokeObjectURL(url);c.toBlob(b=>{if(!b)return flashX('Не вдалося створити PNG');const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='lumen-natal-classic.png';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1500)},'image/png')};
...
function printClassicX(svgMarkup){
  ...
  const style=document.createElement('style');style.id='xNatalPrintStyle';style.textContent='@media print{html,body{background:#fff!important;margin:0!important;padding:0!important}body>:not(#xNatalPrintSheet){display:none!important}#xNatalPrintSheet{display:block!important;width:100%!important;background:#fff!important}#xNatalPrintSheet svg{display:block!important;width:100%!important;height:auto!important;max-height:none!important}@page{size:A4 portrait;margin:7mm}}';
