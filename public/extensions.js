...
function natalHouseX(lon,asc){if(asc==null||!Number.isFinite(lon))return'—';return Math.floor(normX(lon-asc)/30)+1}
...
function classicPositionsTableX(pos,asc=null,mc=null){
  const rows=Object.entries(pos).map(([k,v])=>{const d=degreeMinuteX(v);return`<tr><td><span class="classic-obj">${PM[k][1]}</span> ${PM[k][0]}</td><td>${d.z.glyph} ${d.z.name}</td><td>${String(d.deg).padStart(2,'0')}°${String(d.min).padStart(2,'0')}′</td><td>${natalHouseX(v,asc)}</td></tr>`}).join('');
...
function classicWheelX(pos,asc=null,mc=null,houses=[]){
...
document.addEventListener('click',e=>{
  ...
  if(el.id==='xNCalc'){e.preventDefault();const p={date:document.querySelector('#xNDate').value,time:document.querySelector('#xNTime').value,place:document.querySelector('#xNPlace').value.trim(),utcOffset:document.querySelector('#xNOffset').value,lat:document.querySelector('#xNLat').value,lon:document.querySelector('#xNLon').value};if(!p.date||!p.time||p.utcOffset==='')return flashX('Вкажи дату, час і UTC-зсув');if(p.lat!==''&&(Number(p.lat)<-90||Number(p.lat)>90))return flashX('Широта має бути від -90 до 90');if(p.lon!==''&&(Number(p.lon)<-180||Number(p.lon)>180))return flashX('Довгота має бути від -180 до 180');localStorage.setItem(NKEY,JSON.stringify(p));natalX();return}
