...
function classicPositionsTableX(pos,asc=null,mc=null,houseMeta=null){
  ...
  const rows=Object.entries(pos).map(([k,v])=>{const d=degreeMinuteX(v);return`<tr><td><span class="classic-obj">${PM[k][1]}</span> ${PM[k][0]}</td><td>${classicGlyphX(d.z.glyph)} ${d.z.name}</td><td>${String(d.deg).padStart(2,'0')}°${String(d.min).padStart(2,'0')}′</td><td>${natalHouseX(v,cusps)}</td></tr>`}).join('');
  const angles=`${asc!=null?(()=>{const d=degreeMinuteX(asc);return`<tr class="classic-angle-row"><td><b>AC</b> Асцендент</td><td>${classicGlyphX(d.z.glyph)} ${d.z.name}</td><td>${String(d.deg).padStart(2,'0')}°${String(d.min).padStart(2,'0')}′</td><td>1</td></tr>`})():''}${mc!=null?(()=>{const d=degreeMinuteX(mc);return`<tr class="classic-angle-row"><td><b>MC</b> Середина неба</td><td>${classicGlyphX(d.z.glyph)} ${d.z.name}</td><td>${String(d.deg).padStart(2,'0')}°${String(d.min).padStart(2,'0')}′</td><td>—</td></tr>`})():''}`;
...
function classicWheelX(pos,asc=null,mc=null,houses=[],houseMeta=null,engineLabel='Local fallback'){
  ...
  const axisTag=(lon,label)=>{
    ...
    return `<text x="${p.x.toFixed(1)}" y="${p.y.toFixed(1)}" text-anchor="${anchor}" dominant-baseline="middle" font-size="14" font-weight="800" fill="#111" class="classic-svg-halo classic-svg-halo-5">${label}</text>`
  };
  ...
  for(let i=0;i<12;i++){
    ...
    signLabels+=`<text x="${mid.x.toFixed(1)}" y="${mid.y.toFixed(1)}" text-anchor="middle" dominant-baseline="middle" font-size="25" font-family="Georgia, 'Times New Roman', 'Segoe UI Symbol', serif" fill="#111">${classicGlyphX(Z[i][1])}</text>`;
  }
  if(asc!=null&&houses.length===12){
    houses.forEach((v,i)=>{
      ...
      houseLabels+=`<text x="${mid.x.toFixed(1)}" y="${mid.y.toFixed(1)}" text-anchor="middle" dominant-baseline="middle" font-size="13" font-weight="700" fill="#555" class="classic-svg-halo classic-svg-halo-4">${i+1}</text>`;
    });
  ...
  entries.forEach(([k,v],i)=>{
    ...
    planetMarks+=`<g><text x="${p.x.toFixed(1)}" y="${p.y.toFixed(1)}" text-anchor="middle" dominant-baseline="middle" font-size="22" font-family="serif" font-weight="700" fill="${classicPlanetColorX(k)}" class="classic-svg-halo classic-svg-halo-5">${PM[k][1]}</text><text x="${p.x.toFixed(1)}" y="${(p.y+16).toFixed(1)}" text-anchor="middle" font-size="9" font-weight="700" fill="${classicPlanetColorX(k)}" class="classic-svg-halo classic-svg-halo-3">${deg}°${String(min).padStart(2,'0')}′</text></g>`;
  });
...
function natalX(){activeNav('');const p=parseX(NKEY,{}),lat=Number(p.lat),lon=Number(p.lon),hasGeo=p.lat!==''&&p.lon!==''&&Number.isFinite(lat)&&Number.isFinite(lon)&&Math.abs(lat)<=90&&Math.abs(lon)<=180,tzToken=timezoneTokenX(p),timezoneReady=hasGeo&&p.timezoneToken===tzToken&&!!p.utcIso,b=hasGeo?(timezoneReady?birthX(p):null):birthX(p),houseSystem=p.houseSystem==='placidus'?'placidus':'equal',view=localStorage.getItem('la_natal_view')==='classic'?'classic':'lumen',c=shellX('Натальна карта','Планети, Асцендент, будинки й аспекти · Premium Beta');let result='';window.LUMEN_NATAL_EXPORT_DATA=null;if(b){const jd=b.getTime()/86400000+2440587.5,resolved=resolvedPlanetsX(b,jd),pos=resolved.pos,engineLabel=resolved.engine,asc=hasGeo?ascendantX(jd,lat,lon):null,mc=hasGeo?mcX(jd,lon):null,houseMeta=resolveHouseSystemX(houseSystem,jd,lat,lon,asc,mc),houses=houseMeta.cusps,asp=aspectsX(pos);window.LUMEN_NATAL_EXPORT_DATA={profile:{...p},pos:{...pos},asc,mc,houseMeta:{...houseMeta,cusps:[...houses]},engineLabel};result=`<section class="premium-panel lumen-mt-14"><div class="badge">NATAL ENGINE · BETA · PREMIUM</div><h2>Натальне колесо</h2><p class="lumen-muted-copy">Планетні довготи: ${resolved.precise?'Astronomy Engine · true ecliptic of date':resolved.pending?'завантажується Astronomy Engine; тимчасово показано локальний fallback':'серверні ефемериди недоступні; використано локальний fallback'}. ${asc==null?'Для Асцендента й 12 будинків введи координати місця народження.':houseMeta.fallback?'Placidus недоступний для цих координат або часу — автоматично використано Equal House.':`Будинки: система ${houseSystemStatusX(houseMeta)}.`}</p><div class="natal-view-switch"><button class="${view==='lumen'?'primary':'ghost'}" id="xNatalLumen">LUMEN View</button><button class="${view==='classic'?'primary':'ghost'}" id="xNatalClassic">Classic Chart</button></div>${view==='classic'?classicWheelX(pos,asc,mc,houses,houseMeta,engineLabel)+classicPositionsTableX(pos,asc,mc,houseMeta)+classicAspectMatrixX(pos):wheelX(pos,asc)}<div class="result-actions"><button class="ghost" id="xPng">Зберегти PNG</button><button class="primary" id="xPrint">PDF / Друк</button></div></section><section class="settings-grid">${Object.entries(pos).map(([k,v])=>{const z=zpX(v);return `<article><small>${PM[k][1]} ${PM[k][0].toUpperCase()}</small><h3>${z.glyph} ${z.name}</h3><p>${z.degree.toFixed(1)}° у знаку · ${v.toFixed(1)}° екліптики</p></article>`}).join('')}</section>${asc!=null?`<section class="settings-grid lumen-mt-14"><article><small>ASC</small><h3>${zpX(asc).glyph} ${zpX(asc).name}</h3><p>${zpX(asc).degree.toFixed(1)}° · Асцендент</p></article>${houses.map((v,i)=>{const z=zpX(v);return `<article><small>ДІМ ${i+1}</small><h3>${z.glyph} ${z.name}</h3><p>${z.degree.toFixed(1)}° · ${houseSystemStatusX(houseMeta)}</p></article>`}).join('')}</section>`:''}<section class="premium-panel lumen-mt-14"><div class="badge">АСПЕКТИ</div><h2>Основні зв’язки</h2><div class="reading-cards">${asp.length?asp.map(a=>`<article><div class="card-no">${a.name.toUpperCase()}</div><h3>${PM[a.a][1]} ${PM[a.a][0]} — ${PM[a.b][1]} ${PM[a.b][0]}</h3><p>Орбіс ${a.orb.toFixed(1)}°</p></article>`).join(''):'<article><p>У межах заданих орбісів основні аспекти не знайдені.</p></article>'}</div></section>`}c.innerHTML=`<section class="premium-panel"><div class="badge">FULL NATAL · BETA · PREMIUM</div><div class="settings-grid natal-input-grid"><article><small>ДАТА</small><input id="xNDate" type="date" value="${escX(p.date||'')}" class="lumen-field"></article><article><small>ЧАС</small><input id="xNTime" type="time" value="${escX(p.time||'')}" class="lumen-field"></article><article><small>МІСЦЕ</small><input id="xNPlace" value="${escX(p.place||'')}" class="lumen-field"></article><article><small>UTC · AUTO</small><input id="xNOffset" type="text" readonly value="${escX(timezoneReady?(Number(p.utcOffset)>=0?'+':'')+p.utcOffset:'визначається…')}" class="lumen-field lumen-field-dim"><p class="lumen-help-copy">${timezoneReady?`${escX(p.timezone||'')} · ${escX(p.utcIso||'')}`:'Історичний UTC визначається автоматично за датою та координатами.'}</p></article><article><small>ШИРОТА</small><input id="xNLat" type="number" min="-90" max="90" step="0.0001" value="${escX(p.lat??'')}" placeholder="50.95" class="lumen-field"></article><article><small>ДОВГОТА</small><input id="xNLon" type="number" min="-180" max="180" step="0.0001" value="${escX(p.lon??'')}" placeholder="28.64" class="lumen-field"></article><article><small>СИСТЕМА ДОМІВ</small><select id="xNHouseSystem" class="lumen-field"><option value="equal" ${houseSystem==='equal'?'selected':''}>Equal House</option><option value="placidus" ${houseSystem==='placidus'?'selected':''}>Placidus</option></select><p class="lumen-help-copy">Якщо Placidus неможливо коректно розрахувати для цих координат і часу, автоматично використовується Equal House.</p></article></div><button class="primary wide" id="xNCalc">Зберегти й побудувати</button></section>${result}`;if(hasGeo&&!timezoneReady&&!NATAL_TZ_PENDING_X.has(tzToken))resolveTimezoneX(p);if(b&&!NATAL_EPHEMERIS_CACHE_X.has(ephemerisKeyX(b)))requestEphemerisX(b)}
function classicExportSvgX(data){
  ...
  const W=1400,rowH=36,tableY=965,keys=Object.keys(pos),system=houseSystemStatusX(houseMeta);
  const tableRowsCount=keys.length+(asc!=null?1:0)+(mc!=null?1:0),tableBottom=tableY+88+tableRowsCount*rowH,matrixY=tableBottom+52;
  const infoPrimary=[
  ...
  const cell=58,labelW=160,mx=Math.round((W-(labelW+keys.length*cell))/2),my=matrixY+96,matrixBottom=my+keys.length*cell,H=matrixBottom+92;
  const mHead=keys.map((k,j)=>`<text x="${mx+labelW+j*cell+cell/2}" y="${my-20}" text-anchor="middle" font-size="28" font-family="Georgia, 'Times New Roman', serif" fill="#111">${PM[k][1]}</text>`).join('');
  const mRows=keys.map((a,i)=>{const label=`<text x="${mx}" y="${my+i*cell+38}" font-size="23" font-family="Georgia, 'Times New Roman', serif" fill="#111">${PM[a][1]}</text><text x="${mx+36}" y="${my+i*cell+38}" font-size="14" fill="#555">${escX(PM[a][0])}</text>`;const cells=keys.map((b,j)=>{const x=mx+labelW+j*cell,y=my+i*cell;if(j>=i)return`<rect x="${x}" y="${y}" width="${cell}" height="${cell}" fill="#f7f7f7" stroke="#d5d5d5"/>`;const hit=classicAspectHitX(pos[a],pos[b]);if(!hit)return`<rect x="${x}" y="${y}" width="${cell}" height="${cell}" fill="#fff" stroke="#d5d5d5"/><text x="${x+cell/2}" y="${y+38}" text-anchor="middle" font-size="20" fill="#bbb">·</text>`;const color=hit.tone==='blue'?'#1747ff':hit.tone==='red'?'#ef1717':'#222';return`<rect x="${x}" y="${y}" width="${cell}" height="${cell}" fill="#fff" stroke="#d5d5d5"/><text x="${x+cell/2}" y="${y+31}" text-anchor="middle" font-size="26" font-family="Georgia, 'Times New Roman', serif" font-weight="700" fill="${color}">${hit.symbol}</text><text x="${x+cell/2}" y="${y+49}" text-anchor="middle" font-size="11" fill="${color}">${hit.orb.toFixed(1)}°</text>`}).join('');return label+cells}).join('');
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}"><rect width="${W}" height="${H}" fill="#fff"/><text x="90" y="66" font-size="18" letter-spacing="4" font-weight="700" fill="#8b6b2d">LUMEN ARCANA · CLASSIC NATAL CHART</text><text x="90" y="112" font-size="34" font-family="Georgia,serif" fill="#111">Натальна карта</text><text x="90" y="145" font-size="16" fill="#444">${infoPrimary||'Персональні дані не вказані'}</text><text x="90" y="173" font-size="14" fill="#555">${infoGeo}</text><text x="90" y="201" font-size="14" fill="#666">Система домів: ${escX(system)} · планетний engine: ${escX(engineLabel)}</text><line x1="90" y1="222" x2="1310" y2="222" stroke="#bfa363"/><svg x="320" y="218" width="760" height="760" viewBox="0 0 720 720">${wheel.innerHTML}</svg><text x="90" y="${tableY}" font-size="25" font-family="Georgia,serif" fill="#111">Положення планет</text><text x="90" y="${tableY+42}" font-size="14" font-weight="700" fill="#666">ОБ’ЄКТ</text><text x="520" y="${tableY+42}" font-size="14" font-weight="700" fill="#666">ЗНАК</text><text x="850" y="${tableY+42}" font-size="14" font-weight="700" fill="#666">ПОЗИЦІЯ</text><text x="1190" y="${tableY+42}" font-size="14" font-weight="700" fill="#666">ДІМ</text><line x1="90" y1="${tableY+55}" x2="1310" y2="${tableY+55}" stroke="#888"/>${rows}${angleRows}<text x="90" y="${matrixY}" font-size="28" font-family="Georgia,serif" fill="#111">Матриця аспектів</text><text x="90" y="${matrixY+36}" font-size="14" fill="#1747ff">✶ △ гармонійні</text><text x="300" y="${matrixY+36}" font-size="14" fill="#ef1717">□ ☍ напружені</text><text x="515" y="${matrixY+36}" font-size="14" fill="#222">☌ з’єднання</text>${mHead}${mRows}<text x="90" y="${H-36}" font-size="12" fill="#777">LUMEN ARCANA · ${escX(system)} · Export</text></svg>`
}
...
document.addEventListener('click',e=>{
  ...
  if(el.id==='xClear'){e.preventDefault();if(confirm('Очистити локальні дані LUMEN ARCANA?')){[HKEY,FKEY,NKEY,PKEY,'la_compatibility','la_tarot_type','la_mode','la_sound_enabled','la_sound_volume','la_natal_view','la_personalization_enabled'].forEach(k=>localStorage.removeItem(k));location.reload()}return}
