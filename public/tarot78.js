...
async function handleDraw78(){const prefs=tarotState78(),q=document.querySelector('#q')?.value.trim()||'Що мені важливо побачити зараз?',count=prefs.count||1,cards=draw78(count),content=document.querySelector('#content');if(!content)return;content.innerHTML=`<section class="drawing"><div class="badge">LUMEN · 78 CARDS</div><h2>Карти відкрито</h2><div class="cards">${cards.map(x=>`<div class="tarot tarot-classic"><span>${tarotEsc(x.position)}</span><div class="tarot-art"><img class="rws-card-img ${x.orientation==='reversed'?'is-reversed':''}" src="${x.image}" alt="${tarotEsc(x.name)}" decoding="async"><b class="tarot-art-fallback" style="display:none">${x.symbol}</b></div><strong>${tarotEsc(x.name)}</strong><small>${tarotEsc(x.group)} · ${x.orientation==='reversed'?'перевернута':'пряма'}</small></div>`).join('')}</div><p>Створюю тлумачення…</p></section>`;try{const type=prefs.type,spread=type==='themed'?prefs.spread:type,mode=prefs.mode,headers={'content-type':'application/json'},initData=window.Telegram?.WebApp?.initData||'';if(initData)headers['x-telegram-init-data']=initData;const r=await fetch('/api/interpret',{method:'POST',headers,body:JSON.stringify({language:'uk',question:q,spread,mode,cards,personalization:personalApi78()})});if(!r.ok)throw new Error(`interpret_${r.status}`);const data=await r.json();render78Result(data,q,cards,type)}catch{const p=personal78(),ctx=p.enabled&&p.themes.length?` У твоїй збереженій історії також повторюється тема «${p.themes[0]}»; використай це лише як контекст для роздумів.`:'';render78Result({title:'Твій розклад',synthesis:`Подивись на спільну тему карт і визнач один практичний крок, який залежить від тебе.${ctx}`,cards:cards.map(c=>({card:c.name,symbolism:c.keywords.join(' · '),practice:'Зафіксуй одну дію, яку можеш зробити сьогодні.'}))},q,cards,type)}}
